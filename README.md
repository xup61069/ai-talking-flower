...
- **🌸 動態花花立繪互動**：內建生動的動態 SVG 花花角色，具備呼吸擺動、眨眼、思考與對齊音訊的開闔嘴型（mouth flap）；點擊花花立繪或按鈕可進行「戳戳互動」，觸發可愛跳躍動畫與專屬語音回應。
- **🎭 多重性格預設切換**：一鍵切換「元氣日常花花 🌸」、「暖心夜間花花 🌙」、「辦公摸魚花花 💼」、「幽默吐槽花花 🧠」等多種臺灣日常口語風格，即時調整人設提示詞、語速與創意度。
- **⏰ 定時提醒事項**：內建定時排程引擎，可設定幾分鐘或幾小時後提醒（如喝水、休息、下班），到期時花花會主動語音播報並跳出通知。
- **📊 即時性能與延遲監控**：頂部儀表板即時顯示 ASR 辨識耗時、LLM 首字生成延遲（TTFT）、TTS 首段發聲延遲（TTFA）及全回合總耗時。
- **🧠 記憶搜尋與單筆管理**：即時關鍵字搜尋對話歷史，支援單筆訊息精準刪除、全量匯出或一鍵清空；超過 `recent_turns` 的舊訊息會自動濃縮成背景摘要常駐。
